...
const calculoNotas = async (client, materiaId, alunos, release) => {
  ...
  const calcMedia = (previousV, currentV, index) => previousV
  + (currentV - previousV)
  / (index + 1);
  ...
  for (const aluno of alunos) {
    ...
    const notaBim1 = rows.filter((r) => filtroB1(r.data_atividade, config))
      .map(mapNota).reduce(calcMedia, 0);
    const notaBim2 = rows.filter((r) => filtroB2(r.data_atividade, config))
      .map(mapNota).reduce(calcMedia, 0);
    const notaBim3 = rows.filter((r) => filtroB3(r.data_atividade, config))
      .map(mapNota).reduce(calcMedia, 0);
    const notaBim4 = rows.filter((r) => filtroB4(r.data_atividade, config))
      .map(mapNota).reduce(calcMedia, 0);
...
const calculoFrequencias = async (client, materiaId, alunos, release) => {
  ...
  for (const aluno of alunos) {
    ...
    and materia = $3 and aluno = $4`, [config.inicio_1bimestre, config.fim_4bimestre, materiaId, aluno]);
    const qtdeFaltaBim1 = rows.filter((r) => filtroB1(r.data_presenca, config))
      .filter(filtroNaoPresenca).length;
    const qtdeFaltaBim2 = rows.filter((r) => filtroB2(r.data_presenca, config))
      .filter(filtroNaoPresenca).length;
    const qtdeFaltaBim3 = rows.filter((r) => filtroB3(r.data_presenca, config))
      .filter(filtroNaoPresenca).length;
    const qtdeFaltaBim4 = rows.filter((r) => filtroB4(r.data_presenca, config))
      .filter(filtroNaoPresenca).length;
